...
void incoporacao_ajuste(int N, int M, double Hx, double Hy, double Hz, double **B, double *gxx, double *gxy, double *gxz, double *gyy, double *gyz, double *gzz, double *g, double *Tobs, double *T, double *grad, double **H);
...
double fobj_ajuste (int N, double *Tobs, double *T);
...
int main () {
	...
	double *Y, *X, *Z;
	double *p, *p_inicial, *dp, inclinacao, declinacao, intensidade;
	double *grad, **H, *g;
	double lambida_inicial, lambida, dlambida, fobj1, fobj2, variacao_relativa, epsilon;
	double stdev_dados;
	...
	/* leitura do arquivo da aproximação inicial e parâmetros da inversão ==> */
		...
		fscanf(entrada, "%lf %lf %lf", &lambida_inicial, &dlambida, &epsilon);
	...
	do {
		...
		incoporacao_ajuste(N, M, Hx, Hy, Hz, B, gxx, gxy, gxz, gyy, gyz, gzz, g, Tobs, T, grad, H);
