...
void incoporacao_ajuste(int N, int M, double Hx, double Hy, double Hz, double **B, double *gxx, double *gxy, double *gxz, double *gyy, double *gyz, double *gzz, double *g, double *b, double *p, double *Tobs, double *T, double *grad, double **H, double *residuo);
...
int main () {
	...
	double *gxx, *gxy, *gxz, *gyy, *gyz, *gzz, *Tobs, *T, **B, *b;
	double *Y, *X, *Z;
	...
	b = aloca_vetor_double (relatorio, N);
	...
	do {
		...
		do {
			...
			incoporacao_ajuste(N, M, Hx, Hy, Hz, B, gxx, gxy, gxz, gyy, gyz, gzz, g, b, p, Tobs, T, grad, H, residuo);
...
void incoporacao_ajuste(int N, int M, double Hx, double Hy, double Hz, double **B, double *gxx, double *gxy, double *gxz, double *gyy, double *gyz, double *gzz, double *g, double *b, double *p, double *Tobs, double *T, double *grad, double **H, double *residuo) {
	...
	/* Cálculo da parcela proveniente Jacobiana dos dados ==> */
	for (i = 0; i < N; i++) {
		...
		baux[0] = 1.0;
		baux[1] = -((double)(aux0/aux3))*p[0];
		baux[2] = -((((double)(aux0/aux3))*p[1]) + (((double)(aux1/aux3))*p[0]));
		baux[3] = -((((double)(aux0/aux3))*p[2]) + (((double)(aux2/aux3))*p[0]));
		baux[4] = -((double)(aux1/aux3))*p[1];
		baux[5] = -((((double)(aux1/aux3))*p[2]) + (((double)(aux2/aux3))*p[1]));
		baux[6] = -((double)(aux2/aux3))*p[2];
		
		b[i] = 0.0;
		
		for (j = 0; j < 7; j++) {
		
			b[i] += pow((baux[j]*baux[j]), -1);
	...
	for (i = 0; i < N; i++) { /* for i */
		...
		for (j = 0; j < M; j++) { /* for j */
			...
			grad[j] -= g[j]*b[i]*((double)((Tobs[i] - T[i])/residuo[i]));
			
			for (k = 0; k <= j; k++) { /* for k */
			
				H[k][j] += ((double)((g[k]*b[i]*g[j])/residuo[i]));
